...
settings to manually move students from waiting list to reservation list. This change will only affect your processes.
</p>
<hr>
</div>
<div class="row">
...
<select id="Watingsettings" name="Watingsettings" class="form-control">
  ...
  if($status->manual_waiting_list == 0){
   ...
   } else{echo '<option selected>Automatic</option><option>Manual</option>';}
   echo'
...
// 
    ...
    $("#saveSettings").on('click', function () {
      ...
      $.ajax({
        ...
        error:function(){ ("#failbadge").fadeIn().delay(3000).fadeOut()},
