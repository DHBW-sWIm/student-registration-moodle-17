...
<div class="form">
  ...
  <input type="number" id="tileorder" style="none" class="form-control" placeholder="Order">
...
$table->attributes['style'] = 'font-size: 13px !important;';

$records = $DB->get_records_select("sr_management_tiles",'');

  $table->head = array('ID','Title', 'Button Name','btn URL' , 'Icon' ,'Element 1','Element 2','Element 3','Element 4','Color','Order');
  $table->align = array('left', 'left', 'left','left','left','left','left','left','left','left','left');
  
  foreach($records as $record){

    $table->data[] = array($record->id  ,$record->title , $record->button_name ,$record->button_url ,$record->button_icon , $record->list_element_1, $record->list_element_2, $record->list_element_3, $record->list_element_4, $record->color, $record->tile_order);
