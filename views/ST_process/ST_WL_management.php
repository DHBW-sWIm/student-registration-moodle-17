...
if(has_capability('local/student_registration:manage', $context)){
    ...
    $PAGE->requires->jquery();
        ...
        echo $OUTPUT->heading('Waiting List Management for '.$programname->program_name.' '.$fieldname->study_field_name);
        echo '<br>';
        ...
        $sql = "SELECT company_name, com.id AS ID , count(*) AS total
                FROM {dg_company} AS com
                INNER JOIN {dg_company_representative} AS cr ON com.id = cr.compnay_id
                INNER JOIN {sr_reservation_list} wl ON cr.mdl_user_id = wl.sr_company_representative_id
                {$where}
                GROUP BY company_name
                {$sort}";
        ...
        $table->head = array('Company Name', 'Total Seats Reserved' , 'Total Records on Waiting List' , 'Manage Reservation List' , 'Manage Waiting List');
