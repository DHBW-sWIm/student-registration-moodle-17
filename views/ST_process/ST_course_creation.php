...
$table->head = array('Study course name', 'Abbreviation', 'Study Field',  'Start date', 'End date', 'Course capacity');
...
foreach ($records as $record) {
  ...
  if (isset($DB->get_record_select('sr_study_fields', 'id = ?', array($record->sr_study_fields_id))->study_field_name)) {
    $cell6->text = $DB->get_record_select('sr_study_fields', 'id = ?', array($record->sr_study_fields_id))->study_field_name;
  } else {
    $cell6->text = '';
  }
  ...
  if (new DateTime($record->start_date) < new DateTime("now")) {
    ...
    $padg = '<h5><b><span class="badge badge-pill badge-info">' . $record->start_date . ' </span></b></h5>';
  }
  $cell3->text = $padg;
  ...
  $row->cells  = array($cell1, $cell2, $cell6, $cell3, $cell4, $cell5);
