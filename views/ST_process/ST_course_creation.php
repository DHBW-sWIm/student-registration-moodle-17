...
if ($_SERVER['REQUEST_METHOD']=='POST') {
  ...
  $PAGE->navbar->add('Student Registration',new moodle_url('/local/student_registration/views/ST_process/ST_active_process_Ma_CC.php'));
  echo $OUTPUT->header();
  ...
  $records = $DB->get_records_select("sr_active_study_course",'sr_employees_id = ? AND sr_process_id = ?' ,array($user, $ProcessID) );
      ...
      foreach($records as $record) {
        ...
        $padg = '<b><span class="badge badge-pill badge-success">'.$record->start_date.'</span></b>';
        $cell3->text =$padg;
  ...
  echo '<div id="process_id" hidden >'.$ProcessID.'</div>';
  ?>
    ...
    $('#my-table tr[courseID]').each(function(){
        $(this).css('cursor','pointer').hover(
            ...
            $(this).removeClass('active'); 
                ...
                '<input type="hidden" name="process_id" value="' + ProcessID + '"></input>' + '</form>');
                $('body').append(form);
                $(form).submit();                
