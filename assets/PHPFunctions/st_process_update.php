...
require_once(dirname(dirname(__DIR__)) . '/assets/PHPMailer/process_update_mail.php');
global $DB, $CFG, $USER;
require_login();
$user = $USER->id;
...
if (isset($input['sendMail']) && $input['processclose'] == 'false') {
  ...
  notify_cr_pu('', $demandstart, $regstarta, $regstartb, (new DateTime($input['processend']))->format('Y-m-d H:m:s'), $input['studyprogram']);
} else {

  try {

    if ($input['processclose'] == 'true') {
      $processclose = 1;
    } else {
      $processclose = 0;
    };
    ...
    $DB->update_record('sr_process', array(
      'id' => $id, 'program_name' => $studyprogram,
