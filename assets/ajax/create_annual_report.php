...
// logic for annual report creation
...
try {
    ...
    $edata .= '<td>Demand Difference</td>';
    $edata .= '<td>Reservation Difference</td>';
    $edata .= '</tr></thead>';
    ...
    if (count($recordsjoinRLNew) >= count($companydemandNew && count($recordsjoinRLNew) >= count($recordsjoinRLOld)
        ...
        foreach ($recordsjoinRLNew as $item) {
            $diffdemand = (int)($companydemandNew[$item->company_name]->totaldemand) - (int)($companydemandOld[$item->company_name]->totaldemand);
            ($diffdemand >= 0) ? $badg1 = '<span class="badge badge-info">' . $diffdemand . '</span>' : $badg1 = '<span class="badge badge-danger">' . $diffdemand . '</span>';
            $difreser = (int)($item->total) - (int)($recordsjoinRLOld[$item->company_name]->total);
            ($difreser >= 0) ? $badg2 = '<span class="badge badge-info">' . $difreser . '</span>' : $badg2 = '<span class="badge badge-danger">' . $difreser . '</span>';
            $edata .= '<tr>';
        ...
        if (count($companydemandNew) >= count($recordsjoinRLOld) && count($companydemandNew) >= count($companydemandOld)) {
            foreach ($companydemandNew as $item) {
                $diffdemand = (int)($item->totaldemand - (int)$companydemandOld[$item->company_name]->totaldemand);
                ($diffdemand >= 0) ? $badg1 = '<span class="badge badge-info">' . $diffdemand . '</span>' : $badg1 = '<span class="badge badge-danger">' . $diffdemand . '</span>';
                $difreser = (int)($recordsjoinRLNew[$item->company_name]->total) - (int)($recordsjoinRLOld[$item->company_name]->total);
                ($difreser >= 0) ? $badg2 = '<span class="badge badge-info">' . $difreser . '</span>' : $badg2 = '<span class="badge badge-danger">' . $difreser . '</span>';
                $edata .= '<tr>';
                $edata .= '<td>' . $item->company_name . '</td>';
            ...
            if (count($recordsjoinRLOld) >= count($companydemandOld)) {
                foreach ($recordsjoinRLOld as $item) {
                    $diffdemand = (int)($companydemandNew[$item->company_name]->totaldemand) - (int)($companydemandOld[$item->company_name]->totaldemand);
                    ($diffdemand >= 0) ? $badg1 = '<span class="badge badge-info">' . $diffdemand . '</span>' : $badg1 = '<span class="badge badge-danger">' . $diffdemand . '</span>';
                    $difreser = (int)($recordsjoinRLNew[$item->company_name]->total) - (int)($item->total);
                    ($difreser >= 0) ? $badg2 = '<span class="badge badge-info">' . $difreser . '</span>' : $badg2 = '<span class="badge badge-danger">' . $difreser . '</span>';
                    $edata .= '<tr>';
                    $edata .= '<td>' . $item->company_name . '</td>';
                    $edata .= '<td>' . $companydemandNew[$item->company_name]->totaldemand . '</td>';
                ...
                foreach ($companydemandOld as $item) {
                    $diffdemand = (int)($companydemandNew[$item->company_name]->totaldemand) - (int)($item->totaldemand);
                    ($diffdemand >= 0) ? $badg1 = '<span class="badge badge-info">' . $diffdemand . '</span>' : $badg1 = '<span class="badge badge-danger">' . $diffdemand . '</span>';
                    $difreser = (int)($recordsjoinRLNew[$item->company_name]->total) - (int)($recordsjoinRLOld[$item->company_name]->total);
                    ($difreser >= 0) ? $badg2 = '<span class="badge badge-info">' . $difreser . '</span>' : $badg2 = '<span class="badge badge-danger">' . $difreser . '</span>';
                    $edata .= '<tr>';
