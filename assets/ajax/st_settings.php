      ...
      foreach($activestudyfields as $activestudyfield){
        ...
        if($countRL == 0 || empty($countRL)){
          ...
          for($i= 0 ; $i <  $diff ; $i++){
             ...
             $maxwl = $DB->get_records_select("sr_waiting_list",'sr_process_id = ? AND sr_study_fields_id = ? AND moved_to_rl = 0' ,array( $activestudyfield->sr_process_id , $activestudyfield->sr_study_fields_id) , '' , 'max(timecreated)');
