...
//add new record to the reservation list
...
foreach ($records as $result => $record) {
  $edata .= '<tr>
                 ...
                 <td style="text-align:center;">' . $record->timecreated . '</td>
                 <td style="text-align:center;">' . $DB->get_record_select('sr_active_study_course', 'id = ?', array($record->sr_active_study_course_id))->study_course_abbreviation . '</td>
                 <td style="text-align:center;">' . $record->contract_status . '</td>
              </tr>
             ';
};
